...
import ray
ray.shutdown()
ray.init(num_gpus=96,num_cpus=6) #,log_to_driver=False,ignore_reinit_error=True)  #,memory=230*1024*1024*1024)
...
#             l3 = np.mean((res[ix[0],4] - (self.recovered.values[ix]))**2)
            ...
            #objective function
            gtot=wCases*(l1+0.05*dErrorI) + wDeath*(l2+0.2*dErrorD) + wRecov*l3
    ...
    def create_lossSub(self,pointOrig):
        ...
        beta0, beta01, startT, sigma, a, b, d, mu, p = pointOrig        
...
#                 p=0.4
            ...
            l1 = np.mean((res[ix[0],3] - (self.data.values[ix]))**2)
            l2 = np.mean((res[ix[0],5] - self.death.values[ix])**2)
            deltaRecSq=(np.divide(res[ix[0],4],res[ix[0],3])-0.85)**2
            l3 = np.mean(deltaRecSq)
#             l3 = np.mean((res[ix[0],4] - (self.recovered.values[ix]))**2)
    ...
    def predict(self, point):
        ...
        beta0, beta01, startT, sigma, a, b, d, mu, p, sub, subDth  = point
...
#             (1/120, .4),(1e-12, .4),(1e-12, .4),(1e-12, .4),(1e-12, .4),(1e-12, .4)]
        bounds=[(1e-16, .2),(1e-16, .2),(5,size-5),
            (1/120, .4),(1e-16, .4),(1e-16, .4),(1e-16, .4),(1e-16, .4),(0,1)]
