...
class Learner(object):
    ...
    def create_lossOdeint(self):
        ...
        def lossOdeint(point):
            size = len(self.data)
            beta0, beta01, startT, beta2, sigma,  a, b, d, mu = point
            gamma=a+b
            gamma2=d
            ...
            l3 = np.mean((np.divide(res[ix[0],5],res[ix[0],3])-0.85)**2)
#             l3 = np.mean((res[ix[0],4] - (self.recovered.values[ix]))**2)
            ...
            if self.Deaths:
                ...
                correctGtot=(dNeg)**2*np.sign(dNeg)
    ...
    def create_lossSub(self,p):
        ...
        beta0, beta01, startT, beta2, sigma, a, b, d, mu = p        
...
#             l3 = np.mean((res[ix[0],4] - (self.recovered.values[ix]))**2)
    ...
    def predict(self, p):
        ...
        beta0, beta01, startT, beta2, sigma, a, b, d, mu, sub, subRec, subDth  = p
...
#             (1/120, .4),(1e-12, .4),(1e-12, .4),(1e-12, .4),(1e-12, .4),(1e-12, .4)]
        bounds=[(1e-16, .2),(1e-16, .2),(5,size-5),(1e-16, .2),
            (1/120, .4),(1e-16, .4),(1e-16, .4),(1e-16, .4),(1e-16, .4)]
