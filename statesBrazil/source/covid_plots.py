...
def covid_plots(state, state4Plot,\
                startdate="2020-03-15",predict_range = 60, \
                    startCase = 180, opt = 5, version = "1", show = False, ratio=0.85):
    ...
    df1=df1.select_dtypes(exclude=['object', 'datetime']) * ratio
    df=df1.subtract(df2)
    ...
    if opt==5 or opt==0:
        ...
        extended_actual = np.int32(actual.values*ratio)-np.int32(death.values)
