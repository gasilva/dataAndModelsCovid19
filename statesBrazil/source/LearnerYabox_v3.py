...
class Learner(object):
    ...
    def create_lossOdeint(self,data, \
                death, s_0, e_0, a_0, i_0, r_0, d_0, startNCases, \
                     ratioRecovered,weigthCases, weigthRecov, twoPoints):

        def lossOdeint(point):
            size = len(data)
            t0, t1, beta0, beta1, beta01, beta2, sigma, sigma2, sigma3,  gamma, b, gamma2, d, mu = point
            def SEAIRD(y,t,tmax):
                beta=sg.betaX(t,tmax,beta0,beta1,beta01,t0,t1,twoPoints)
                S = y[0]
                E = y[1]
                A = y[2]
                I = y[3]
            ...
            tmax=size
            res=odeint(SEAIRD,y0,tspan,args=(tmax,)) #,hmax=0.01)
    ...
    def predict(self, t0, t1, beta0, beta1, beta01, beta2, sigma, sigma2, sigma3,  gamma, \
                b, gamma2, d, mu, data, \
                    death, state, s_0, e_0, a_0, i_0, r_0, d_0, twoPoints):
        ...
        tmax=size
        res=odeint(SEAIRD,y0,tspan,args=(tmax,))
    ...
    def train(self):
        ...
        if twoPoints:
            bounds=[(5,int(tmax*0.5)-10),(int(tmax*0.5)+5,tmax-5),\
                    (1e-12, .2), (1e-12, .2), (1e-12, .2), (1e-12, .2), (1/120 ,0.4),(1/120, .4),
            (1/120, .4),(1e-12, .4),(1e-12, .4),(1e-12, .4),(1e-12, .4),(1e-12, .4)]
        else:
            bounds=[(5,tmax-5),(0,0),\
                    (1e-12, .2), (1e-12, .2), (0,0), (1e-12, .2), (1/120 ,0.4),(1/120, .4),
            (1/120, .4),(1e-12, .4),(1e-12, .4),(1e-12, .4),(1e-12, .4),(1e-12, .4)]

        maxiterations=3500
        f=self.create_lossOdeint(self.data, \
            self.death, self.s_0, self.e_0, self.a_0, self.i_0, self.r_0, self.d_0, self.startNCases, \
                 self.ratio, self.weigthCases, self.weigthRecov, twoPoints)
        ...
        t0, t1, beta0, beta1, beta01, beta2, sigma, sigma2, sigma3, gamma, b, gamma2, d, mu  = p

        new_index, extended_actual, extended_death, y0, y1, y2, y3, y4, y5 \
                = self.predict(t0, t1, beta0, beta1, beta01, beta2, sigma, \
                               sigma2, sigma3, gamma, b, gamma2, d, mu, \
                    self.data, self.death, self.state, self.s_0, \
                    self.e_0, self.a_0, self.i_0, self.r_0, self.d_0, twoPoints)
