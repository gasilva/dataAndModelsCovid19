...
class Learner(object):
    ...
    def load_confirmed(self, state):
        ...
        df = pd.read_csv('./data/CasosConfirmadosEstadosBrasil.csv',delimiter=',',parse_dates=True, date_parser=dateparse)
        y=[]
        x=[]
    ...
    def load_dead(self, state):
        dateparse = lambda x: datetime.strptime(x, '%Y-%m-%d')
        df = pd.read_csv('./data/MortosEstadosBrasil.csv',delimiter=',',parse_dates=True, date_parser=dateparse)
        y=[]
        x=[]
        for i in range(0,len(df.date)):
            y.append(df[state].values[i])
